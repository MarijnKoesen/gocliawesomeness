...
// Handler is a custom slog tintedHandler that wraps another slog.Handler to prepend a prefix to the log
...
type Handler struct {
	...
	opts     HandlerOptions // Configuration options for this tintedHandler.
	prefixes []slog.Value   // Cached list of prefix values.
}

var _ slog.Handler = (*Handler)(nil)

// NewTintedHandler creates a new prefix logging tintedHandler.
// The new tintedHandler will prepend a prefix sourced from the log record's attributes to each log
// message before passing the record to the next tintedHandler.
...
// record to the next tintedHandler.
