...
// NewTintedHandler creates a [slog.Handler] that writes tinted logs to Writer w,
...
// tintedHandler implements a [slog.Handler].
type tintedHandler struct {
	attrsPrefix string
	groupPrefix string
	groups      []string
...
func (h *tintedHandler) clone() *tintedHandler {
	return &tintedHandler{
...
func (h *tintedHandler) Enabled(_ context.Context, level slog.Level) bool {
	return level >= h.level.Level()
}

func (h *tintedHandler) Handle(_ context.Context, r slog.Record) error {
	...
	// write tintedHandler attributes
...
func (h *tintedHandler) WithAttrs(attrs []slog.Attr) slog.Handler {
	if len(attrs) == 0 {
		return h
	}
	h2 := h.clone()
...
func (h *tintedHandler) WithGroup(name string) slog.Handler {
	if name == "" {
		return h
	}
	h2 := h.clone()
...
func (h *tintedHandler) appendTime(buf *buffer, t time.Time) {
	buf.WriteStringIf(!h.noColor, ansiFaint)
	*buf = t.AppendFormat(*buf, h.timeFormat)
	buf.WriteStringIf(!h.noColor, ansiReset)
}

func (h *tintedHandler) appendLevel(buf *buffer, level slog.Level) {
	switch {
...
func (h *tintedHandler) appendSource(buf *buffer, src *slog.Source) {
...
func (h *tintedHandler) appendAttr(buf *buffer, attr slog.Attr, groupsPrefix string, groups []string) {
...
func (h *tintedHandler) appendKey(buf *buffer, key, groups string) {
	buf.WriteStringIf(!h.noColor, ansiFaint)
...
func (h *tintedHandler) appendValue(buf *buffer, v slog.Value, quote bool) {
	switch v.Kind() {
...
func (h *tintedHandler) appendTintError(buf *buffer, err error, groupsPrefix string) {
